...
import sys, os
sys.path.insert(0,"/mnt/CONIRepo/ykkim/SLCL/base_experiments/TSCIL")
...
class SingleHeadModel(nn.Module):
    def __init__(self, encoder, head, input_channels, feature_dims, n_layers, seq_len, n_base_nodes, norm, input_norm, dropout):
        ...
        if input_norm == 'LN':
            ...
            # Model now receives (N, C, T) channels-first (encoder no longer transposes).
            # nn.InstanceNorm1d(C) normalises (N, C, T) over T per channel — no transpose needed.
            self.input_norm = nn.InstanceNorm1d(input_channels, affine=False)
        else:
